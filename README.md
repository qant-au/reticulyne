...
Reticulyne renders node `description` fields as HTML through a **TipTap** rich-text editor. Its schema is the XSS boundary, so the containment is strong by construction — but there is still one thing that stays your responsibility:

- ✅ **What the library does for you.** The editor registers only five inline marks (`bold`, `italic`, `underline`, `strike`, `link`). Every `description` — whether typed, pasted, or supplied via the `value`/`initialData` prop — is parsed through a schema built from just those marks, so any other tag or attribute is dropped: `<script>`, `<iframe srcdoc>`, `<svg onload>`, `<img onerror>`, `<style>`, inline `on*` handlers. This runs on the way **in** (before anything renders) and again on the way **out** (read-only display is regenerated purely from the schema). Links get an extra pass: a `SafeLink` extension rejects `javascript:`, `data:`, `vbscript:`, `file:`, and `blob:` URLs (including percent-encoded variants) on every `<a href>`.
- ❌ **What stays your responsibility.** Only what you do with a `description` **outside** Reticulyne. A value returned by `onModelUpdated` is already schema-normalised HTML, but if you render it in your own UI, feed it to a different HTML sink, or store it and later trust it verbatim, sanitise it there like any other user data.

DOMPurifying `initialData` before passing it in is therefore now **optional hardening** rather than required — the editor re-parses the value through the schema before rendering. If you still want belt-and-braces (e.g. your `description` strings flow through other systems), [DOMPurify](https://github.com/cure53/DOMPurify) remains a drop-in:
