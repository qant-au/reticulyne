...
| `exportTheme` | `'light'` \| `'dark'` | `'light'` | Controls the initial background colour in the export dialog (PNG / PDF). `'light'` produces a white-background image; `'dark'` uses the dark-mode diagram background. The user can still change the background colour inside the dialog before downloading. |
