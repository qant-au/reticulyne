...
describe('handleImportedJsonText (BUG5-05)', () => {
  ...
  test('logs an [reticulyne] error and skips load() when the text is not valid JSON', () => {
    ...
    expect(errorSpy.mock.calls[0][0]).toMatch(
      /\[reticulyne\] imported file is not valid JSON/
    );
  });
  ...
  test('logs an [reticulyne] error and skips load() when the input is null', () => {
    ...
    expect(errorSpy).toHaveBeenCalled();
    expect(errorSpy.mock.calls[0][0]).toMatch(
      /\[reticulyne\] imported file could not be read as text/
    );
  });
});
